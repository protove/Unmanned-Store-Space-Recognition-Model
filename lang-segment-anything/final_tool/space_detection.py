...
def process_ice_cream_clusters(farthest_cluster, other_top_clusters, depth_image, top3_draw, classified_draw, contour_draw):
    ...
    # 아이스크림 디스플레이 관련 클러스터 식별
    ice_cream_clusters = []
    remaining_clusters = []
    
    for cluster in other_top_clusters:
        # 클러스터에 포함된 모든 마스크 확인
        is_ice_cream_cluster = False
        
        # 클러스터 내 마스크 순회
        for mask_info in cluster['masks']:
            # 마스크가 어떤 프롬프트로 생성되었는지 확인
            if 'prompt' in mask_info and any(prompt in mask_info['prompt'] for prompt in ice_cream_prompts):
                is_ice_cream_cluster = True
                break
